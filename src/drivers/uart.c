...
void sys_uart0_init(void) {
  ...
  write32(addr + UART_DLL,
          0xd & 0xff); // 0x0d=13 240000000/(13*16) = 115200 Divisor Latch Lows
  ...
  val = read32(addr + UART_LCR);
  val &= ~0x1f;
  val |= (0x3 << 0) | (0 << 2) | (0x0 << 3); // 8 bit, 1 stop bit,parity
                                             // disabled
  write32(addr + UART_LCR, val);
...
void _putchar(char character) { sys_uart_putc(0, character); }