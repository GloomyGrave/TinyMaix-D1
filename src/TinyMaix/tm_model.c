...
tm_load(tm_mdl_t *mdl, const uint8_t *bin, uint8_t *buf, tm_cb_t cb,
        tm_mat_t *in, uint8_t *m_buf, uint8_t *sub_buf) {
...
// remove model
void __attribute__((weak)) tm_unload(tm_mdl_t *mdl) { return; }
  ...
  for (mdl->layer_i = 0; mdl->layer_i < mdl->b->layer_cnt; mdl->layer_i++) {
    ...
    case TML_DWCONV2D: {
      ...
      res = tml_conv2d_dwconv2d(
          ...
          l->pad[0], l->pad[1], l->pad[2], l->pad[3], l->depth_mul,
